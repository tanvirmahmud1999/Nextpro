...
class Custom_Colors
{
	...
	public function __construct()
	{
		...
		if (is_customize_preview()) {
			add_action('wp_enqueue_scripts', array($this, 'editor_custom_color_variables'));
		}
	...
	public function custom_color_variables()
	{
		if ('d1e4dd' !== strtolower(get_theme_mod('background_color', 'D1E4DD'))) {
			wp_add_inline_style('nextpro-style', $this->generate_custom_color_variables());
		}
		$colors = nextpro_get_default_colors_global();
		$colors_css =  [];
		$prefix = 'bs-';
		foreach ($colors as $color) {
			$color_mod = get_theme_mod('nextpro_' . $color['id'], $color['value']);
			if (!empty($color_mod) && strtolower($color['value']) != strtolower($color_mod)) {
				$color_mod_rgb = $this->hex2rgb($color_mod, false);
				$color_name = str_replace('_', '-', $color['id']);
				$colors_css[] = "--{$prefix}{$color_name}: {$color_mod};";
				$colors_css[] = "--{$prefix}{$color_name}-rgb: {$color_mod_rgb};";
			}
		}
		if (!empty(array_filter($colors_css))) {
			wp_add_inline_style('nextpro-style', ':root{' . implode(' ', $colors_css) . '}');
		}
	}
	...
	{
		wp_enqueue_style(
			'nextpro-customizer-preview',
			get_theme_file_uri('assets/css/customizer-preview.css'),
			array(),
		...
		$background_color = get_theme_mod('background_color', 'D1E4DD');
		if ('d1e4dd' !== strtolower($background_color)) {
			wp_add_inline_style('nextpro-customizer-preview', $this->generate_custom_color_variables('editor'));
		}
	}
	...
	public function hex2rgb($color, $opacity = '1')
	{
		$color = trim($color, '#');

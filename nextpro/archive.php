<?php
$blog_archive_column = intval(get_theme_mod('blog_archive_column', 2));
if ($blog_archive_column > 0) {
	$blog_archive_column = intval(12 / $blog_archive_column);
}
...
if (have_posts()) { ?>
	...
	while (have_posts()) {
		the_post();
		get_template_part('template-parts/content/' . nextpro_get_post_content_style(), get_post_format(), ['column_class' => $blog_archive_column]);
	}
